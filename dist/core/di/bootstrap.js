...
export async function bootstrapApplication() {
    ...
    container.singleton(TOKENS.MetricsCollector, (c) => new MetricsCollector(c.resolve(TOKENS.EventBus)));
    container.singleton(TOKENS.PerformanceMonitor, (c) => new PerformanceMonitor(c.resolve(TOKENS.EventBus), c.resolve(TOKENS.MetricsCollector)));
    ...
    container.singleton(TOKENS.SecurityManager, (c) => new SecurityManager(c.resolve(TOKENS.AnomalyDetector), c.resolve(TOKENS.InputValidator), c.resolve(TOKENS.AuditLogger), c.resolve(TOKENS.SecureDataHandler)));
    // Register business services
    container.singleton(TOKENS.Storage, (c) => new WhatsAppStorageService(c.resolve(TOKENS.EventBus), c.resolve(TOKENS.Logger), c.resolve(TOKENS.SecurityManager)));
    container.singleton(TOKENS.Extractor, (c) => new WhatsAppExtractorService(c.resolve(TOKENS.EventBus), c.resolve(TOKENS.Logger), c.resolve(TOKENS.SecurityManager), c.resolve(TOKENS.Storage), c.resolve(TOKENS.MetricsCollector)));
    container.singleton(TOKENS.UIService, (c) => new UIService(c.resolve(TOKENS.EventBus), c.resolve(TOKENS.Logger), c.resolve(TOKENS.Storage), c.resolve(TOKENS.MetricsCollector), c.resolve(TOKENS.ConfigService)));
    // Create service registry
    const serviceRegistry = new ServiceRegistry(container.resolve(TOKENS.EventBus), container.resolve(TOKENS.Logger));
    ...
    window.__appContainer = container;
    return {
        container,
        serviceRegistry
    };
